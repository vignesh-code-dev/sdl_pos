  ...
  const POSBilling = () => {
    ...
    const hasActiveCreditAccount = (() => {
      if (!customerMobile || customerMobile.trim().length !== 10 || isNewCustomer) {
        return false;
      }
      try {
        const rawAccounts = localStorage.getItem("billmate_deposit_accounts");
        const accountsList = rawAccounts ? JSON.parse(rawAccounts) : [];
        const foundAccount = accountsList.find(acc => acc.customerMobile === customerMobile);
        return !!foundAccount;
      } catch (e) {
        return false;
      }
    ...
    useEffect(() => {
      if (paymentMethod && paymentMethod !== "CREDIT") {
        ...
        if (Math.abs(parsedTender - prevGrandTotalRef.current) < 0.01 || tenderedAmount === "") {
     ...
     const handleCompleteSale = () => {
      ...
      const tenderVal = parseFloat(tenderedAmount) !== undefined && !isNaN(parseFloat(tenderedAmount))
        ? parseFloat(tenderedAmount)
        : (paymentMethod === "CREDIT" ? 0 : grandTotal);

      const balanceValue = paymentMethod === "CREDIT" ? grandTotal : Math.max(0, grandTotal - tenderVal);
      const changeValue = paymentMethod === "CREDIT" ? 0 : Math.max(0, tenderVal - grandTotal);
      const finalPaidAmount = paymentMethod === "CREDIT" ? 0 : tenderVal;

      if (balanceValue > 0 && !hasActiveCreditAccount) {
        const errorMsg = "Outstanding balances or partial payments are only allowed for customers with an active credit account. Please create or activate a credit account first.";
                  ...
                  {paymentOptions.map(opt => {
                    ...
                    return (
                      <button key={opt.value} type="button"
                        onClick={() => {
                          if (isOptDisabled) {
                            showToast("This customer does not have an active credit account. Please create or activate a credit account before using CREDIT payment.", "warning");
                            return;
                          }
                        ...
                        <span className={isOptDisabled ? "text-gray-300" : paymentMethod === opt.value ? "text-emerald-600" : "text-slate-400"}>{opt.icon}</span>
                        <span>{opt.label}</span>
                      </button>
                    );
                  })}
                      ...
                      <div className="mt-2 flex justify-between text-[11px] font-semibold text-slate-500 font-mono">
                        {parseFloat(tenderedAmount) > grandTotal ? (
                          <>
                            ...
                            <span className="text-emerald-600 font-bold">₹{(parseFloat(tenderedAmount) - grandTotal).toFixed(2)}</span>
                          </>
                        ) : parseFloat(tenderedAmount) < grandTotal ? (
                          <>
                            <span>Outstanding:</span>
                            <span className="text-rose-600 font-bold">₹{(grandTotal - (parseFloat(tenderedAmount) || 0)).toFixed(2)}</span>
                          </>
                        ) : (
                          <>
                            <span>Outstanding:</span>
