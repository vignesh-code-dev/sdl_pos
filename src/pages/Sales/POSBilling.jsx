...
const POSBilling = () => {
  ...
  const parseSearchInput = (input) => {
    ...
    if (trimmed.includes("*")) {
      ...
      if (parts.length >= 2) {
        ...
        const product0 = products.find(
          (p) =>
            p.sku.toLowerCase() === part0.toLowerCase() ||
            p.name.toLowerCase() === part0.toLowerCase(),
        );
        const product1 = products.find(
          (p) =>
            p.sku.toLowerCase() === part1.toLowerCase() ||
            p.name.toLowerCase() === part1.toLowerCase(),
        );
        ...
        if (product0 && !product1) {
          ...
          return {
            targetSku: part0,
            targetQty: isDec ? parseFloat(part1) || 1 : parseInt(part1) || 1,
          };
        }
        if (product1 && !product0) {
          ...
          return {
            targetSku: part1,
            targetQty: isDec ? parseFloat(part0) || 1 : parseInt(part0) || 1,
          };
        }

        // 2. If one of them is a partial SKU/name match (includes)
        const partial0 = products.find(
          (p) =>
            p.sku.toLowerCase().includes(part0.toLowerCase()) ||
            p.name.toLowerCase().includes(part0.toLowerCase()),
        );
        const partial1 = products.find(
          (p) =>
            p.sku.toLowerCase().includes(part1.toLowerCase()) ||
            p.name.toLowerCase().includes(part1.toLowerCase()),
        );
  ...
  const { targetSku: suggestSku, targetQty: suggestQty } =
  ...
  const addItemToCart = (product, qty) => {
    ...
    setCart((prevCart) => {
      ...
      const currentRate =
        parseFloat(product.sellingPrice) || parseFloat(product.rate) || 0;
  ...
  const handleProductSearchSubmit = (e) => {
    ...
    if (!product) {
      ...
      if (partials.length === 1) {
        ...
        const matchNoSkuPrefix = partials.find(
          (p) =>
            p.sku.toLowerCase().replace(/^sku/, "") === targetSku.toLowerCase(),
        );
        if (matchNoSkuPrefix) {
    ...
    if (product) {
      ...
      if (
        ...
        showToast(
          `Product with SKU or Barcode '${targetSku}' was not found in database.`,
          "warning",
        );
      }
    }
        ...
        if (item.sku === sku) {
          ...
          if (updatedQty >= minQty) {
            return {
              ...item,
              quantity: isDecimal
                ? parseFloat(updatedQty.toFixed(3))
                : Math.round(updatedQty),
            };
          }
        }
        return item;
  ...
  const subtotal = cart.reduce(
    (sum, item) => sum + item.rate * (parseFloat(item.quantity) || 0),
    0,
  );
  const totalLineDiscount = cart.reduce(
    (sum, item) =>
      sum +
      item.rate * (parseFloat(item.quantity) || 0) * (item.discount / 100),
    0,
  );
  ...
  const handleCompleteSale = () => {
    if (cart.length === 0) {
      showToast(
        "The shopping cart is empty! Please scan or search products first.",
        "warning",
      );
      return;
    }
    ...
    const receiptObj = {
      ...
      customerName: customerRefName,
      customerMobile: customerMobile || "Walk-In",
      items: cart.map((item) => ({
        ...item,
    ...
    if (paymentMethod === "CREDIT") {
      try {
        ...
        let targetIndex = accountsList.findIndex(
          (acc) =>
            acc.customerMobile === identifier ||
            (receiptObj.customerMobile &&
              acc.customerMobile === receiptObj.customerMobile),
        );
              ...
              <div className="absolute top-13 left-0 right-24 bg-white border border-gray-200 rounded-lg shadow-lg z-50 overflow-hidden">
                ...
                {filteredSuggestions.map((p, idx) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-emerald-600 font-semibold ml-2">
                      ₹{parseFloat(p.sellingPrice || p.rate || 0).toFixed(2)}
                    </div>
                  </div>
                ))}
              </div>
              ...
              {cart.length === 0 ? (
                ...
                <div>
                  {cart.map((item, idx) => (
                    <div
                      key={item.sku}
                      className="cart-row border-b border-gray-100 last:border-b-0 px-6 py-4.5 grid grid-cols-12 gap-4 items-center hover:bg-slate-50/40 transition-colors"
                    >
                      <div className="col-span-1 text-[15px] text-gray-800 font-bold text-center">
                        {idx + 1}
                      </div>
                      <div className="col-span-3 text-left">
                        ...
                        <div className="text-[11px] text-slate-500 mt-0.5 font-semibold uppercase tracking-wider">
                          {item.sku}
                        </div>
                      </div>
                      <div className="col-span-1 text-center text-[16px] font-bold text-slate-700">
                        {item.rate.toFixed(2)}
                      </div>
                        ...
                        <div className="flex items-center justify-center">
                          <div className="flex items-center border border-gray-250 rounded bg-white h-8 overflow-hidden shadow-2xs hover:border-gray-400 transition-colors">
                            ...
                            <input
                              type="number"
                              min={
                                isDecimalUnit(item.unit || item.unitType)
                                  ? "0.001"
                                  : "1"
                              }
                              step={
                              ...
                              value={item.quantity}
                              onChange={(e) =>
                                handleQtyDirectChange(item.sku, e.target.value)
                              }
                      ...
                      <div className="col-span-1 text-center text-[14px] font-bold text-slate-500 uppercase tracking-wider">
                        {item.unit || "PCS"}
                      </div>
                      ...
                      <div className="col-span-1 text-center text-[16px] text-slate-700 font-bold">
                        {(item.tax || 0).toFixed(2)}
                      </div>
                      <div className="col-span-1 pr-1 text-right text-[16px] font-bold text-slate-800">
          ...
          <div className="flex-1 overflow-y-auto pos-scroll p-4 space-y-4">
                ...
                <div>
                  <div className="relative">
                    ...
                    {customerMobile.length === 10 && !isNewCustomer && (
                      <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[9px] font-extrabold bg-emerald-50 border border-emerald-200 text-emerald-700 px-1.5 py-0.5 rounded uppercase">
                        Linked
                      </span>
                    )}
                    {customerMobile.length === 10 && isNewCustomer && (
                      <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[9px] font-extrabold bg-amber-50 border border-amber-200 text-amber-700 px-1.5 py-0.5 rounded uppercase">
                        New
                      </span>
                    )}
                  </div>
                    ...
                    <div className="animate-in fade-in slide-in-from-top-1 duration-150">
                      <div className="relative">
                        ...
                        <div className="w-full h-9 bg-emerald-50/50 border border-emerald-100 rounded pl-9 pr-4 flex items-center justify-between text-[13px] font-bold text-slate-800">
                          ...
                          <span className=" absolute right-2.5 top-1/2 -translate-y-1/2 text-[9px] rounded text-emerald-700 font-extrabold uppercase border bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 ">
                            Active
                          </span>
                        </div>
                      </div>
            ...
            <div className="bg-slate-50/80 rounded p-4 space-y-2 border border-slate-150">
              ...
              <div className="flex justify-between items-center border-t border-slate-250 pt-3 ">
                <span className="text-[16px] font-black text-slate-700 uppercase">
                  GRAND PAYABLE
                </span>
