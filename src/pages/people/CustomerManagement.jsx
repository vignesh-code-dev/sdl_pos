...
const CustomerManagement = () => {
    ...
    if (saved) {
      try {
        ...
        if (parsed && parsed.length > 0) {
          ...
          if (!hasRahul) {
            const newSeed = [
              { id: "C-1", name: "Rahul Sharma", mobile: "9876543210", email: "rahul@gmail.com", address: "123, MG Road, Indore, MP 452001", createdAt: "2026-01-15", status: "Active" },
              { id: "C-2", name: "Priya Patel", mobile: "8765432109", email: "priya@gmail.com", address: "45, Palasia Square, Indore, MP 452001", createdAt: "2026-02-18", status: "Active" },
              { id: "C-3", name: "Amit Kumar", mobile: "7654321098", email: "amit@gmail.com", address: "78, Vijay Nagar, Indore, MP 452001", createdAt: "2026-03-05", status: "Active" },
              { id: "C-4", name: "Ram", mobile: "9786595211", email: "-", address: "-", createdAt: "2026-06-08", status: "Active" },
              { id: "C-5", name: "Jay", mobile: "8527845618", email: "-", address: "-", createdAt: "2026-06-09", status: "Active" },
              { id: "C-6", name: "jay", mobile: "8795421135", email: "-", address: "-", createdAt: "2026-06-09", status: "Active" },
              ...parsed.filter(p => !["Jane Cooper", "Floyd Miles", "Ronald Richards", "Marvin McKinney", "Jerome Bell", "Kathryn Murphy", "Jacob Jones", "Kristin Watson"].includes(p.name))
            ];
    ...
    if (savedInvoices) {
      try {
        ...
        if (parsed && Array.isArray(parsed) && parsed.length > 0) {
          const sum = parsed.reduce((total, item) => total + (parseFloat(item.grandTotal) || 0), 0);
          setTotals({
  ...
  const handleFormSubmit = (e) => {
    ...
    if (trimmedMobile.length < 7) {
      setValidationError("Customer phone digits should be at least 7 characters.");
      return;
    }
  ...
  const handleExportCSV = () => {
    const csvHeader = "Customer Name,Phone Number,Email,Address,Added On,Status\n";
    ...
    link.href = url;
    link.setAttribute("download", `customers_summary_${new Date().toISOString().split("T")[0]}.csv`);
  ...
  const paginatedList = filteredCustomers.slice(startIndex, startIndex + itemsPerPage);
  ...
  const displayNewCustomers = customers.length <= 6 ? 32 : Math.round(customers.length * 0.15 || 4);
        ...
        <div className="bg-pos-card border border-pos-border rounded-2xl shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              ...
              <tbody className="divide-y divide-pos-border/50 text-[13.5px] text-[#475569]">
                {paginatedList.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="p-20 text-center text-slate-400 font-semibold text-xs bg-white">
                      No customers found matching the selected filters. Register a customer to get started.
                    </td>
                  </tr>
                ) : (
                  paginatedList.map((c) => {
                    ...
                    return (
                      <tr key={c.id} className="hover:bg-slate-50/40 transition-colors text-text-secondary text-[14px]">
                        
                        {/* Logo and Name bubble */}
                        <td className="p-4">
                          <div className="flex items-center gap-3.5">
                            <div className={`w-9 h-9 rounded-full flex items-center justify-center font-extrabold text-[12.5px] tracking-tight border shadow-2xs select-none uppercase shrink-0 ${avatarColorClass}`}>
                              {initials}
                            </div>
                            <span className="font-extrabold text-slate-800 text-[13.5px]">{c.name}</span>
                          </div>
                        </td>
                          ...
                          <div className="flex items-start gap-2 text-slate-500 text-[13px] max-w-[220px]">
                            {c.address && c.address !== "-" ? (
                              <>
                                ...
                                <span className="line-clamp-2 leading-relaxed" title={c.address}>{c.address}</span>
                              </>
                            ) : (
                              <span className="text-slate-300 font-mono select-none">-</span>
                            )}
          ...
          {totalRecords > 0 && (
            <div className="bg-slate-50/50 p-4 border-t border-pos-border flex flex-col sm:flex-row justify-between items-center text-xs font-bold text-slate-600 gap-2 select-none">
              <span>Showing data <span className="font-extrabold text-slate-700">{startIndex + 1}</span> to <span className="font-extrabold text-slate-700">{Math.min(totalRecords, startIndex + itemsPerPage)}</span> of <span className="font-extrabold text-slate-700">{totalRecords}</span> entries</span>
                ...
                {Array.from({ length: totalPages }, (_, idx) => idx + 1).map((page) => {
                  ...
                  if (totalPages > 5 && !isFirst && !isLast && !isNearCurrent) {
                    if (page === 2 && currentPage > 3) {
                      return <span key="ellipsis-start" className="px-1 text-slate-300 font-extrabold select-none">...</span>;
                    }
                    if (page === totalPages - 1 && currentPage < totalPages - 2) {
                      return <span key="ellipsis-end" className="px-1 text-slate-300 font-extrabold select-none">...</span>;
                    }
                    return null;
                  }
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-extrabold text-[14px] text-slate-900 uppercase tracking-wider">
                    {modalMode === "add" ? "Register Customer" : "Update Customer Profile"}
                  </h3>
                  <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide">Secure Contact File</p>
                </div>
              </div>
              <button
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 border border-pos-border animate-in zoom-in-95 duration-150">
            <h3 className="text-sm font-black text-slate-900 mb-2 uppercase tracking-wide flex items-center gap-1.5 select-none">
              {customConfirm.isDanger ? (
                <span className="text-brand-danger flex items-center gap-1.5">⚠️ Danger Warning</span>
              ) : (
                <span className="text-brand-primary flex items-center gap-1.5">💡 Confirm Task</span>
              )}
            </h3>
